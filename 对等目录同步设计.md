...
1. 每个 Obsidian vault 始终把“当前 vault 根目录”视为本端根目录，把其他配置的目录视为对端根目录，正向同步负责本端 → 对端，反向同步负责对端 → 本端。
2. 对等目录的“一致性”通过两端各自运行的「日志合并（mergeLogsForTargets）+ 正向同步 + 反向同步」循环共同达成：任一端的 `side: "source"` 日志和 mtime 变化都会在两端被感知并参与冲突判断。
3. 冲突的判定基于同一路径上本端/对端的最新未解决事件（source/target 的 modified/deleted 组合），一旦检测到双方都有未处理修改或删除，就不再静默覆盖，而是通过 Notice + diff 视图提示用户处理。
